...
async function login(email, installationId) {
  const result = await api('/auth/login', {
    ...
    body: {
      ...
      device: { installationId, name: `Harness ${installationId}`, platform: 'android', osVersion: '14', appVersion: '1.0.0' },
    },
  });
        ...
        fields: [
          ...
          },
          {
            ...
            logic: [
              {
                id: 'r1',
                when: { kind: 'CONDITION', condition: { fieldId: gateId, operator: 'EQUALS', value: 'no' } },
                effect: { type: 'HIDE' },
              },
            ],
            followUps: [],
...
async function main() {
  ...
  check('signup availability is public and unauthenticated',
  ...
  check('a new organisation sees zero inspections from other tenants',
  ...
  check('a new organisation sees only its own single user',
  ...
  check('the starter checklist is published and immediately usable',
  ...
  const duplicate = await api('/auth/register', {
    ...
    body: {
      ...
      firstName: 'Dup', lastName: 'Licate', organizationName: 'Duplicate Co',
      device: { installationId: `harness-dup-${stamp}`, name: 'H', platform: 'web', osVersion: '1', appVersion: '1.0.0' },
    },
  });
  check('a duplicate email is refused with a pointer to sign in',
  ...
  const weak = await api('/auth/register', {
    ...
    body: {
      ...
      firstName: 'Weak', lastName: 'Pass', organizationName: 'Weak Co',
      device: { installationId: `harness-weak-${stamp}`, name: 'H', platform: 'web', osVersion: '1', appVersion: '1.0.0' },
    },
  });
  ...
  check('first version is a DRAFT, not auto-published',
    created.body?.data?.activeVersionId === null || created.body?.data?.activeVersionId === undefined,
        ...
        sections: [
          {
            ...
            fields: [
              {
                key: 'a', label: 'A', type: 'TEXT', options: [], validation: {}, ui: {}, followUps: [],
                logic: [
                  {
                    id: 'r1',
                    when: { kind: 'CONDITION', condition: { fieldId: ulid(), operator: 'EQUALS', value: 'x' } },
                    effect: { type: 'HIDE' },
                  },
                ],
              },
  ...
  check('definition with a dangling logic reference is rejected', danglingRef.status === 422,
  ...
  check('definition with no sections is rejected', emptyDefinition.status === 422,
  ...
  check('a PUBLISHED version cannot be edited', editPublished.status === 409,
  ...
  check('published template appears in the sync delta',
  ...
  check('a new draft version can be created from the published one',
  ...
  const clone = await api(`/templates/${templateId}/clone`, { method: 'POST', token: manager.token });
  ...
  check('export omits internal ids at the top level',
    !('id' in (exported.body?.template ?? {})), JSON.stringify(exported.body?.template ?? {}).slice(0, 120));

  const imported = await api('/templates/import', {
    method: 'POST',
    token: manager.token,
    body: { ...exported.body, template: { ...exported.body.template, name: 'Imported harness template' } },
  });
  check('template imports', imported.status === 201, JSON.stringify(imported.body).slice(0, 200));
  check('import lands as an unpublished draft',
    !imported.body?.data?.activeVersionId, `activeVersionId=${imported.body?.data?.activeVersionId}`);
  check('import preserved the question count', imported.body?.data?.fieldCount === 2,
  ...
  check('unsupported export format is rejected', badFormat.status === 422, `status=${badFormat.status}`);
  ...
  check('inspector cannot author templates', inspectorWrite.status === 403,
  ...
  check('password hash is never returned',
    (usersList.body?.data?.items ?? []).every((u) => !('passwordHash' in u) && !('passwordHistory' in u)));
  ...
  check('admin can invite a user', invited.status === 201, JSON.stringify(invited.body).slice(0, 200));
  check('invited user has no password and INVITED status',
    invited.body?.data?.status === 'INVITED', `status=${invited.body?.data?.status}`);
  ...
  check('an ADMIN cannot create a SUPER_ADMIN', escalateSuper.status === 403,
  ...
  const adminUserId = admins.body?.data?.items?.find((u) => u.id !== admin.userId)?.id
  ...
  if (adminUserId) {
    ...
    check('a MANAGER cannot modify an ADMIN', managerTouchesAdmin.status === 403,
  ...
  check('unknown permissions are rejected rather than silently stored',
    badPermission.status === 422, `status=${badPermission.status}`);

  const roleMeta = await api('/users/meta/roles', { token: admin.token });
  check('role matrix is exposed for the admin UI', roleMeta.status === 200 &&
    Array.isArray(roleMeta.body?.data?.roles), `status=${roleMeta.status}`);
  check('role matrix marks which roles the caller may assign',
  ...
  check('new client reaches devices via the sync delta',
  ...
  check('project created with a valid client reference', project.status === 201,
  ...
  check('a non-existent client reference is rejected', crossTenant.status === 422,
  ...
  check('an end date before the start date is rejected', badDates.status === 422,
  ...
  check('a geofence without coordinates is rejected as meaningless',
    geofenceNoCoords.status === 422, `status=${geofenceNoCoords.status}`);
  ...
  check('site created with coordinates and geofence', site.status === 201,
    JSON.stringify(site.body).slice(0, 200));

  const cursorBeforeDelete = await currentCursor(inspector.token);
  const deleted = await api(`/sites/${site.body?.data?.id}`, { method: 'DELETE', token: admin.token });
  ...
  check('deletion propagates as a tombstone, not a silent disappearance',
  ...
  check('inspector cannot create clients', inspectorWritesClient.status === 403,
  ...
  check('rates are bounded percentages',
    summary.body?.data?.completionRate >= 0 && summary.body?.data?.completionRate <= 100 &&
    summary.body?.data?.failureRate >= 0 && summary.body?.data?.failureRate <= 100,
    `completion=${summary.body?.data?.completionRate} failure=${summary.body?.data?.failureRate}`);
  check('an empty period yields 0, never NaN',
    Number.isFinite(summary.body?.data?.completionRate));

  const trend = await api('/analytics/trend?period=MONTHLY', { token: admin.token });
  check('trend returns buckets', trend.status === 200 && Array.isArray(trend.body?.data),
    `status=${trend.status}`);
  check('bucket counts are numbers, not BigInt strings',
    (trend.body?.data ?? []).every((b) => typeof b.total === 'number'));

  const inspectors = await api('/analytics/inspectors', { token: admin.token });
  check('inspector performance returns 200', inspectors.status === 200, `status=${inspectors.status}`);
  ...
  const badRange = await api('/analytics/summary?from=2026-12-01T00:00:00.000Z&to=2026-01-01T00:00:00.000Z', {
    token: admin.token,
  });
  ...
  check('CSV carries the expected header', csvText.includes('Number,Title,Template'),
    csvText.slice(0, 80));
  ...
  check('CSV is UTF-8 BOM prefixed for Excel',
    csvBytes[0] === 0xef && csvBytes[1] === 0xbb && csvBytes[2] === 0xbf,
    `first bytes: ${Array.from(csvBytes.slice(0, 3)).map((b) => b.toString(16)).join(' ')}`);

  // --- audit --------------------------------------------------------------
  section('6. Audit and monitoring');

  const audit = await api('/admin/audit-logs?pageSize=10', { token: admin.token });
  check('audit log returns entries', audit.status === 200 && audit.body?.data?.total > 0,
    `status=${audit.status} total=${audit.body?.data?.total}`);

  const auditDenied = await api('/admin/audit-logs', { token: inspector.token });
  check('inspector denied audit access', auditDenied.status === 403, `status=${auditDenied.status}`);

  const health = await api('/admin/sync-health', { token: admin.token });
  check('sync health returns fleet state', health.status === 200 &&
    typeof health.body?.data?.serverCursor === 'number', `status=${health.status}`);
  check('sync health reports how far each device is behind',
  ...
  check('partial settings patch merges rather than replacing',
  ...
  check('the patched value actually changed',
  ...
  check('manager denied organisation settings', settingsDenied.status === 403,
