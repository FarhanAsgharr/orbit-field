...
async function main() {
  ...
  check('identity carries the fields the console reads',
  ...
  check('health carries serverCursor / conflicts / uploads / devices',
    hasKeys(health.data, ['serverCursor', 'unresolvedConflicts', 'pendingUploads', 'devices']) === null,
    hasKeys(health.data, ['serverCursor', 'unresolvedConflicts', 'pendingUploads', 'devices']));
  check('serverCursor is a number the rail can position against',
    typeof health.data?.serverCursor === 'number', `type=${typeof health.data?.serverCursor}`);

  const device = health.data?.devices?.[0];
  if (device) {
    check('each device carries behind / stale / cursor for the lag rail',
      hasKeys(device, ['id', 'name', 'userName', 'behind', 'stale', 'cursor', 'lastSyncAt']) === null,
      hasKeys(device, ['id', 'name', 'userName', 'behind', 'stale', 'cursor', 'lastSyncAt']));
    check('behind is numeric — the rail computes a position from it',
      typeof device.behind === 'number', `type=${typeof device.behind}`);
  } else {
  ...
  check('summary carries every metric the overview renders',
    hasKeys(summary.data, [
      'total', 'statusCounts', 'outcomeCounts', 'completed', 'failed',
      'overdue', 'dueToday', 'completionRate', 'failureRate',
      'averageScore', 'averageDurationMinutes',
    ]) === null,
    hasKeys(summary.data, ['total', 'statusCounts', 'outcomeCounts', 'completionRate', 'failureRate']));
  check('statusCounts is a keyed object the overview iterates',
    summary.data?.statusCounts && typeof summary.data.statusCounts === 'object' && !Array.isArray(summary.data.statusCounts));
  ...
  for (const [path, label] of paginated) {
    ...
    check(`${label} (${path}) returns the pagination envelope`,
  ...
  if (inspection) {
    check('inspection row carries the columns the table renders',
      hasKeys(inspection, [
        'id', 'number', 'title', 'status', 'outcome', 'priority',
        'score', 'answeredFields', 'totalFields', 'dueAt', 'updatedAt',
      ]) === null,
      hasKeys(inspection, ['id', 'number', 'title', 'status', 'outcome', 'answeredFields', 'totalFields']));
    check('inspection row includes joined template / site / client / assignee',
      'template' in inspection && 'site' in inspection && 'client' in inspection && 'assignedTo' in inspection,
      `keys: ${Object.keys(inspection).filter((k) => ['template', 'site', 'client', 'assignedTo'].includes(k)).join(',')}`);
    check('inspection row includes _count for attachments',
  ...
  if (user) {
    check('user row carries the columns the People table renders',
      hasKeys(user, ['id', 'email', 'firstName', 'lastName', 'role', 'status', 'lastLoginAt']) === null,
      hasKeys(user, ['id', 'email', 'firstName', 'lastName', 'role', 'status']));
    check('user row includes device and workload counts',
      user._count && typeof user._count.devices === 'number' && typeof user._count.assignedInspections === 'number',
      JSON.stringify(user._count));
    check('password material is never present',
  ...
  if (template) {
    check('template row carries activeVersionId and versions[] for the published badge',
    ...
    check('template row includes version and usage counts',
      template._count && typeof template._count.versions === 'number' && typeof template._count.inspections === 'number',
  ...
  if (site) {
    check('site row carries coordinates and geofence for the location column',
  ...
  check('GET /devices returns a bare array (not paginated)',
  ...
  if (Array.isArray(devices.data) && devices.data[0]) {
    check('device row carries the columns the table renders',
      hasKeys(devices.data[0], ['id', 'name', 'platform', 'osVersion', 'appVersion', 'lastSeenAt', 'lastSyncAt', 'revokedAt']) === null,
  ...
  check('GET /admin/conflicts returns the pagination envelope',
    conflicts.status === 200 && Array.isArray(conflicts.data?.items),
    `status=${conflicts.status}`);

  const conflict = conflicts.data?.items?.[0];
  if (conflict) {
    check('conflict carries diffs[] the resolution UI iterates',
      Array.isArray(conflict.diffs), `diffs=${typeof conflict.diffs}`);
    check('conflict carries operationId for the resolve call',
      typeof conflict.operationId === 'string');
    const diff = conflict.diffs?.[0];
    if (diff) {
      check('each diff carries path / label / isConflicting / both values',
        hasKeys(diff, ['path', 'label', 'localValue', 'serverValue', 'isConflicting', 'autoResolution']) === null,
  ...
  check('GET /admin/sync-sessions returns the pagination envelope',
    sessions.status === 200 && Array.isArray(sessions.data?.items),
    `status=${sessions.status}`);
  const sess = sessions.data?.items?.[0];
  if (sess) {
    check('sync session carries counts and outcome the table renders',
      hasKeys(sess, ['id', 'trigger', 'pushedCount', 'pulledCount', 'conflictCount', 'outcome', 'startedAt']) === null,
      hasKeys(sess, ['trigger', 'pushedCount', 'pulledCount', 'conflictCount', 'outcome']));
    check('sync session includes joined device and user',
      'device' in sess && 'user' in sess);
  }

  // --- analytics ----------------------------------------------------------
  section('Analytics charts');
  const trend = await api('/analytics/trend', admin, { period: 'WEEKLY' });
  check('GET /analytics/trend returns an array', Array.isArray(trend.data), `type=${typeof trend.data}`);
  if (Array.isArray(trend.data) && trend.data[0]) {
    check('trend point carries bucket + the three plotted series',
      hasKeys(trend.data[0], ['bucket', 'total', 'completed', 'failed']) === null,
      hasKeys(trend.data[0], ['bucket', 'total', 'completed', 'failed']));
    check('trend counts are numbers, not BigInt strings — Recharts cannot plot strings',
      typeof trend.data[0].total === 'number', `type=${typeof trend.data[0].total}`);
  }
  ...
  if (Array.isArray(inspectorRows.data) && inspectorRows.data[0]) {
    check('inspector row carries the columns the table renders',
      hasKeys(inspectorRows.data[0], ['userId', 'name', 'assigned', 'completed', 'averageScore', 'onTimeRate']) === null,
      hasKeys(inspectorRows.data[0], ['userId', 'name', 'assigned', 'completed', 'averageScore', 'onTimeRate']));
  }
  ...
  if (Array.isArray(siteRows.data) && siteRows.data[0]) {
    check('site analytics row carries failureRate for the bar chart',
  ...
  check('organisation carries settings and counts the settings screen renders',
    hasKeys(org.data, ['id', 'name', 'settings', '_count']) === null,
    hasKeys(org.data, ['id', 'name', 'settings', '_count']));
  check('counts include users / devices / sites / inspections',
  ...
  check('an inspector is denied fleet health, so the rail hides those links',
