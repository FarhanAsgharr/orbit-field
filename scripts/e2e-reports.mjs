...
async function main() {
  ...
  check(
    'GET /reports/datasets returns 200',
    catalogue.status === 200,
    `status=${catalogue.status}`,
  );
  check(
    'catalogue lists datasets with formats and availability',
    Array.isArray(catalogue.data) &&
      catalogue.data.every((d) => 'key' in d && 'formats' in d && 'available' in d),
    JSON.stringify(catalogue.data?.[0] ?? {}).slice(0, 120),
  );
  check(
    "catalogue reflects the caller's permissions",
    catalogue.data.some((d) => d.available === true),
  );

  const inspectorCatalogue = await api('/reports/datasets', inspector);
  check(
    'an inspector sees audit exports as unavailable',
  ...
  check(
    'xlsx carries the spreadsheet content type',
    xlsx.contentType.includes('spreadsheetml'),
    xlsx.contentType,
  );
  check(
    'xlsx is served as an attachment with a filename',
    xlsx.disposition.includes('attachment') && xlsx.disposition.includes('.xlsx'),
    xlsx.disposition,
  );
  check(
    'xlsx is not cached — an export is a point-in-time snapshot',
    xlsx.cacheControl.includes('no-store'),
    xlsx.cacheControl,
  );
  check(
    'xlsx is a real ZIP container',
  ...
  check(
    'xlsx contains a worksheet',
    entries.some((e) => e.startsWith('xl/worksheets/sheet')),
    entries.slice(0, 5).join(', '),
  );
  check(
    'xlsx contains styles — headers and signals are formatted',
  ...
  check(
    'csv carries a UTF-8 BOM so Excel opens it correctly',
  ...
  check(
    'csv neutralises formula injection in free-text cells',
    dangerous.length === 0,
  ...
  check(
    ...
    summary.status === 200 && summary.bytes.length > 2000,
  ...
  const batch = await download(
    '/reports/batch?datasets=inspections,users,devices&format=xlsx',
    admin,
  );
  ...
  check(
    'batch workbook has one worksheet per dataset',
    sheetCount === 3,
  ...
  check(
    'an inspector cannot export the audit log',
  ...
  check(
    'an inspector cannot export the user list',
    deniedUsers.status === 403,
    `status=${deniedUsers.status}`,
  );

  const allowedOwn = await download('/reports/export/inspections?format=csv', inspector);
  check(
    'an inspector can export their own inspections',
    allowedOwn.status === 200,
  ...
  check(
    'a batch drops datasets the caller cannot reach rather than failing',
  ...
  check(
    'export history returns entries',
    history.status === 200 && history.data?.total > 0,
    `status=${history.status} total=${history.data?.total}`,
  );
  check(
    'history records the format and row count',
  ...
  check(
    'inbox reports an unread count for the app badge',
    typeof inbox.data?.unread === 'number',
  ...
  check(
    'preferences enumerate every topic with a human label',
    Array.isArray(prefs.data?.topics) &&
      prefs.data.topics.every((t) => t.label && t.label !== t.topic),
    JSON.stringify(prefs.data?.topics?.[0] ?? {}),
  );
  check(
    'conflict alerts are marked unmutable',
  ...
  check(
    ...
    quiet.status === 200 && quiet.data?.quietHours?.startHour === 22,
    JSON.stringify(quiet.data?.quietHours),
  );
  check(
    'a partial preference update preserves the rest',
  ...
  check(
    'conflict alerts cannot be muted',
    muteConflict.status === 422,
  ...
  check(
    'an inspector cannot broadcast an announcement',
    announceDenied.status === 403,
  ...
  const announce = await api('/notifications/announce', admin, {
    ...
    body: {
      ...
      body: 'Sync will pause briefly on Sunday at 02:00.',
      role: 'INSPECTOR',
    },
  });
  check(
    'an admin can broadcast',
    announce.status === 200 && announce.data?.created > 0,
    `status=${announce.status} created=${announce.data?.created}`,
  );

  const afterAnnounce = await api('/notifications?pageSize=5', inspector);
  check(
    'the announcement lands in the inbox even with push unavailable',
  ...
  check(
    'latency histogram is present',
    metricsText.includes('orbit_http_request_duration_seconds_bucket'),
  );
  check(
    'fleet gauges are present',
    metricsText.includes('orbit_unresolved_conflicts') &&
      metricsText.includes('orbit_devices_stale'),
  );
  check(
    'route labels are normalised, not per-id',
