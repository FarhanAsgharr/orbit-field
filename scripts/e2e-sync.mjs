...
async function main() {
  ...
  const badLogin = await api('/auth/login', {
    ...
    body: {
      ...
      device: {
        installationId: 'e2e-bad-credentials-probe',
        name: 'x',
        platform: 'android',
        osVersion: '14',
        appVersion: '1.0.0',
  ...
  check(
    'rejection does not leak whether the account exists',
  ...
  check(
    ...
    JSON.stringify(bootstrap.body).slice(0, 200),
  );
  ...
  check('cursor is a JSON number, not a BigInt string', typeof bootstrap.body?.cursor === 'number');
  ...
  check(
    'replayed operation reported as DUPLICATE',
    replayResult?.status === 'DUPLICATE',
    `status=${replayResult?.status}`,
  );
  check(
    'replay returned the original version',
    replayResult?.version === 1,
  ...
  check(
    'replay did not create a duplicate record',
    matching.length === 1,
  ...
  const alphaEdit = operation(
    alpha,
    'INSPECTION',
    'UPDATE',
    inspectionId,
    { notes: 'Alpha: found corrosion on the busbar' },
    1,
  );
  const alphaEditResult = await push(alpha, [alphaEdit]);
  check(
    'Alpha edit applied',
    alphaEditResult.body?.results?.[0]?.status === 'APPLIED',
    JSON.stringify(alphaEditResult.body?.results?.[0]).slice(0, 200),
  );

  const bravoEdit = operation(
    bravo,
    'INSPECTION',
    'UPDATE',
    inspectionId,
  ...
  check(
    'stale-but-disjoint edit auto-merged rather than conflicting',
    bravoStatus === 'APPLIED',
  ...
  const alphaClash = operation(
    alpha,
    'INSPECTION',
    'UPDATE',
    inspectionId,
    { title: 'Alpha renamed this' },
    currentVersion,
  );
  const alphaClashResult = await push(alpha, [alphaClash]);
  check(
    'Alpha title change applied',
  ...
  const bravoClash = operation(
    bravo,
    'INSPECTION',
    'UPDATE',
    inspectionId,
    { title: 'Bravo renamed this differently' },
    currentVersion,
  );
  ...
  check(
    'clashing edit reported as CONFLICT',
    conflictResult?.status === 'CONFLICT',
    `status=${conflictResult?.status}`,
  );
  check(
    'conflict carries a three-way diff',
    Array.isArray(conflictResult?.conflict?.diffs),
    JSON.stringify(conflictResult?.conflict ?? {}).slice(0, 200),
  );

  const titleDiff = (conflictResult?.conflict?.diffs ?? []).find((d) => d.path === 'title');
  check(
    'the clashing field is flagged as conflicting',
    titleDiff?.isConflicting === true,
  ...
  const stillAlpha = (afterConflict.body?.changes ?? [])
    ...
    .sort((a, b) => b.syncCursor - a.syncCursor)[0];
  check(
    'server retained the winning value, nothing silently lost',
  ...
  check(
    'conflict is persisted server-side for later resolution',
    conflictList.status === 200 &&
      Array.isArray(conflictList.body?.data) &&
  ...
  check(
    'resolution returns the merged record',
  ...
  check(
    'all 25 queued operations applied in one batch',
    appliedCount === 25,
  ...
  check(
    'every inspection received a unique server-allocated number',
    numbers.size >= 25,
    `distinct numbers=${numbers.size}`,
  );
  ...
  if (viewerLogin.status === 429) {
    ...
    check('VIEWER role denied sync:push', viewerPush.status === 403, `status=${viewerPush.status}`);
  } else {
  ...
  if (adminLogin.status !== 200) {
    ...
    check(
      'found the peer inspector and a project',
      Boolean(peer && projectId),
    ...
    if (peer && projectId) {
      ...
      check(
        'peer inspector given a project membership',
        patched.status === 200,
      ...
      if (peerLogin.status === 200) {
        ...
        const foreign = inspections.filter(
          (c) => c.data?.assignedToId && c.data.assignedToId !== peer.id,
        );

        check(
          "project membership does not leak another inspector's work",
          foreign.length === 0,
        ...
        check(
          'reference data still reaches a project-scoped inspector',
