...
describe('templates API', () => {
  it('lists templates for an authenticated user', async () => {
    const res = await request(app).get(`${api}/templates`).set(auth('INSPECTOR'));
...
describe('inspections API', () => {
  it('returns the inspections assigned to the caller', async () => {
    const res = await request(app).get(`${api}/inspections`).set(auth('INSPECTOR'));
    expect(res.status).toBe(200);
  ...
  it('returns a single inspection by id', async () => {
    const res = await request(app).get(`${api}/inspections/${inspectionId}`).set(auth('INSPECTOR'));
    expect(res.status).toBe(200);
  ...
  it('rejects a malformed id rather than treating it as a lookup miss', async () => {
    const res = await request(app).get(`${api}/inspections/not-a-ulid`).set(auth('ADMIN'));
  ...
  it('does not return another organisation’s inspection', async () => {
    ...
    try {
      ...
      const res = await request(app).get(`${api}/inspections/${foreignId}`).set(auth('ADMIN'));
...
describe('reports API', () => {
  ...
  it('lists the available datasets', async () => {
    const res = await request(app).get(`${api}/reports/datasets`).set(auth('ADMIN'));
    expect(res.status).toBe(200);
  });

  it('generates a real PDF, not an error page with a 200', async () => {
    const res = await binary(
      request(app).get(`${api}/reports/inspection/${inspectionId}?format=pdf`).set(auth('ADMIN')),
    );
    expect(res.status).toBe(200);
  ...
  it('generates a real xlsx workbook', async () => {
    const res = await binary(
      request(app).get(`${api}/reports/export/inspections?format=xlsx`).set(auth('ADMIN')),
    );
    expect(res.status).toBe(200);
  ...
  it('refuses report export to a VIEWER without the permission', async () => {
    const res = await request(app).get(`${api}/reports/datasets`).set(auth('INSPECTOR'));
...
describe('analytics API', () => {
  ...
  it.each(endpoints)('%s returns data for an ADMIN', async (path) => {
    expect((await request(app).get(`${api}${path}`).set(auth('ADMIN'))).status).toBe(200);
  });

  it.each(endpoints)('%s is refused for an INSPECTOR', async (path) => {
    expect((await request(app).get(`${api}${path}`).set(auth('INSPECTOR'))).status).toBe(403);
  });
});

describe('devices API', () => {
  it('lists the caller’s enrolled devices', async () => {
    const res = await request(app).get(`${api}/devices`).set(auth('ADMIN'));
    expect(res.status).toBe(200);
  });
...
describe('notifications API', () => {
  it('returns the inbox', async () => {
    expect((await request(app).get(`${api}/notifications`).set(auth('INSPECTOR'))).status).toBe(
      200,
    );
  });
  ...
  it('reads and writes preferences', async () => {
    expect(
      (await request(app).get(`${api}/notifications/preferences`).set(auth('INSPECTOR'))).status,
    ).toBe(200);
...
describe('admin API', () => {
  it('returns the organisation record', async () => {
    const res = await request(app).get(`${api}/admin/organization`).set(auth('ADMIN'));
    expect(res.status).toBe(200);
    expect(res.body.data.id).toBe(org.orgId);
  });

  it('writes an audit trail that an ADMIN can read', async () => {
    const res = await request(app).get(`${api}/admin/audit-logs`).set(auth('ADMIN'));
    expect(res.status).toBe(200);
    expect(Array.isArray(res.body.data.items ?? res.body.data)).toBe(true);
  });
});
...
describe('input validation', () => {
  ...
  it('does not 500 on a SQL-injection attempt in a query parameter', async () => {
    const res = await request(app).get(`${api}/inspections?limit=1' OR '1'='1`).set(auth('ADMIN'));
    expect(res.status).not.toBe(500);
  });

  it('rejects an oversized page size rather than trying to serve it', async () => {
    const res = await request(app).get(`${api}/inspections?limit=99999`).set(auth('ADMIN'));
