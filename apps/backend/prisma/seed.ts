...
async function main(): Promise<void> {
  ...
  const people = [
    { n: 1, role: 'ADMIN' as const, first: 'Amara', last: 'Osei', email: 'admin@northwind.test', title: 'Systems Administrator' },
    { n: 2, role: 'MANAGER' as const, first: 'Ravi', last: 'Chandran', email: 'manager@northwind.test', title: 'Operations Manager' },
    { n: 3, role: 'SUPERVISOR' as const, first: 'Elena', last: 'Petrova', email: 'supervisor@northwind.test', title: 'Field Supervisor' },
    { n: 4, role: 'INSPECTOR' as const, first: 'Tom', last: 'Whitfield', email: 'inspector@northwind.test', title: 'Senior Inspector' },
    { n: 5, role: 'INSPECTOR' as const, first: 'Priya', last: 'Nair', email: 'inspector2@northwind.test', title: 'Inspector' },
    { n: 6, role: 'TECHNICIAN' as const, first: 'Jonas', last: 'Berg', email: 'technician@northwind.test', title: 'Technician' },
    { n: 7, role: 'VIEWER' as const, first: 'Sara', last: 'Lindqvist', email: 'viewer@northwind.test', title: 'Compliance Analyst' },
  ];
    ...
    sections: [
      {
        ...
        fields: [
          {
            id: F(1), sectionId: S1, key: 'safe_to_proceed', label: 'Is it safe to proceed with the inspection?',
            type: 'YES_NO', order: 0, weight: 1, isCritical: true, defaultValue: null, carryForward: false,
            options: [
            ...
            logic: [
              {
                id: 'r1',
                when: { kind: 'CONDITION', condition: { fieldId: F(1), operator: 'EQUALS', value: 'no' } },
                effect: { type: 'REVEAL_FOLLOW_UPS' },
              },
            ],
            followUps: [
              {
                id: F(2), sectionId: S1, key: 'halt_reason', label: 'Describe the hazard that prevented work',
                type: 'TEXT_AREA', order: 0, weight: 1, isCritical: false, defaultValue: null, carryForward: false,
                options: [], ui: {}, logic: [], followUps: [],
                validation: { required: true, minLength: 20 },
              },
            ],
          },
          {
            id: F(3), sectionId: S1, key: 'supply_voltage', label: 'Measured supply voltage (V)',
            type: 'NUMBER', order: 1, weight: 2, isCritical: false, defaultValue: null, carryForward: false,
            options: [], followUps: [],
            ...
            logic: [
              {
                id: 'r2',
                when: { kind: 'CONDITION', condition: { fieldId: F(3), operator: 'GREATER_THAN', value: 253 } },
                effect: { type: 'BLOCK_SUBMIT', message: 'Supply voltage exceeds the statutory limit. Escalate before submitting.' },
              },
            ],
          },
          {
            id: F(4), sectionId: S1, key: 'earthing_system', label: 'Earthing arrangement',
            type: 'DROPDOWN', order: 2, weight: 1, isCritical: false, defaultValue: null, carryForward: true,
            followUps: [], logic: [], ui: {},
            validation: { required: true },
            options: [
          ...
          {
            id: F(5), sectionId: S1, key: 'site_location', label: 'Confirm your location on site',
            type: 'GPS', order: 3, weight: 1, isCritical: false, defaultValue: null, carryForward: false,
            options: [], followUps: [], logic: [], ui: {},
      ...
      },
      {
        ...
        fields: [
          {
            id: F(6), sectionId: S2, key: 'enclosure_condition', label: 'Enclosure condition and IP rating intact',
            type: 'PASS_FAIL', order: 0, weight: 2, isCritical: false, defaultValue: null, carryForward: false,
            followUps: [
              {
                id: F(7), sectionId: S2, key: 'enclosure_defect_photo', label: 'Photograph the defect',
                type: 'PHOTO', order: 0, weight: 1, isCritical: false, defaultValue: null, carryForward: false,
                options: [], logic: [], followUps: [],
                validation: { required: true, minAttachments: 1, maxAttachments: 5, requiredGpsAccuracyMeters: 100 },
            ...
            logic: [
              {
                id: 'r3',
                when: { kind: 'CONDITION', condition: { fieldId: F(6), operator: 'EQUALS', value: 'fail' } },
                effect: { type: 'REVEAL_FOLLOW_UPS' },
              },
            ],
          },
          {
            id: F(8), sectionId: S2, key: 'rcd_trip_time', label: 'RCD trip time at IΔn (ms)',
            type: 'NUMBER', order: 1, weight: 3, isCritical: true, defaultValue: null, carryForward: false,
            options: [], followUps: [], ui: { helpText: 'Must not exceed 300 ms for a 30 mA RCD.' },
            validation: { required: true, min: 0, max: 300, precision: 1 },
            logic: [],
          },
          {
            id: F(9), sectionId: S2, key: 'defect_categories', label: 'Observed defect categories',
            type: 'MULTI_SELECT', order: 2, weight: 1, isCritical: false, defaultValue: null, carryForward: false,
            followUps: [], logic: [], ui: {},
            validation: {},
            options: [
          ...
          {
            id: F(10), sectionId: S2, key: 'overall_rating', label: 'Overall condition rating',
            type: 'RATING', order: 3, weight: 2, isCritical: false, defaultValue: null, carryForward: false,
            options: [], followUps: [], logic: [],
            validation: { required: true },
            ui: { ratingMin: 1, ratingMax: 5, ratingIcon: 'STAR' },
          },
          {
            id: F(11), sectionId: S2, key: 'inspector_signature', label: 'Inspector',
            type: 'SIGNATURE', order: 4, weight: 1, isCritical: false, defaultValue: null, carryForward: false,
            options: [], followUps: [], logic: [], ui: {},
