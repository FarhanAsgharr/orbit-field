 ...
 *
 * The eye exists because passwords are mistyped constantly on a keyboard you
 * cannot see, and the alternative — a "confirm password" field — asks somebody
 * to type the same thing twice without ever seeing either. Revealing is the
 * better trade, and it matters more here than on the console: a customer
 * registering is choosing a password against a strength policy they are
 * meeting for the first time, and a rejection they cannot see the cause of is
 * where people give up.
 *
 * This does not use `Field` from `ui.tsx`, deliberately. That wraps its
 * children in a `<label>`, and a `<button>` inside a label inherits the
 * label's activation — clicking the eye would also click the input. So the
 * markup is spelled out here with an explicit `htmlFor`, which is the same
 * thing minus the nested-interactive problem.
 *
 * Three details that decide whether this helps or annoys:
 *
 *  - The toggle is a real `<button type="button">`. A `<span>` with an onClick
 *    is invisible to keyboards and screen readers, and a button without an
 *    explicit type submits the form it sits in — which on the registration
 *    page would submit eighteen half-filled fields.
 ...
 *  - Revealed text reverts to hidden on blur, so a password is not left
 *    readable on a screen somebody walked away from.
 */
...
> {
  ...
  error?: string | null;
  hint?: string;
  /** Span the full width of a `.form-grid`, as `Field` does with `full`. */
  full?: boolean;
...
export const PasswordField = forwardRef<HTMLInputElement, PasswordFieldProps>(
  function PasswordField(
    { label, error, hint, full, persistReveal = false, id, className, onBlur, ...rest },
    ref,
  ) {
    ...
    return (
      <div className={full ? 'field form-grid--full' : 'field'}>
        ...
        <div className={error ? 'password password--error' : 'password'}>
          <input
            {...rest}
            ref={ref}
            id={inputId}
