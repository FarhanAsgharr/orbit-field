...
interface Summary {
  total: number; completed: number; failed: number; overdue: number; dueToday: number;
  completionRate: number; failureRate: number;
  averageScore: number | null; averageDurationMinutes: number | null;
  statusCounts: Record<string, number>;
}

interface TrendPoint { bucket: string; total: number; completed: number; failed: number; passed: number }
interface InspectorRow {
  userId: string; name: string; assigned: number; completed: number; failed: number;
  completionRate: number; averageScore: number | null; averageDurationMinutes: number | null;
  onTimeRate: number | null;
}
interface SiteRow {
  siteId: string; name: string; total: number; failed: number; failureRate: number;
  averageScore: number | null; lastInspectedAt: string | null;
}
...
export function Analytics(): React.ReactElement {
  ...
  return (
    <>
      <header className="page__head">
        ...
        <div className="row gap-3 wrap">
          <select className="select" style={{ width: 'auto' }} value={days}
            onChange={(e) => setDays(Number(e.target.value))} aria-label="Period">
            <option value={30}>Last 30 days</option>
            <option value={90}>Last 90 days</option>
            <option value={365}>Last year</option>
          </select>
          <select className="select" style={{ width: 'auto' }} value={period}
            onChange={(e) => setPeriod(e.target.value as Period)} aria-label="Grouping">
      ...
      {summary.isError ? (
        <ErrorBanner message={summary.error instanceof Error ? summary.error.message : 'Could not load analytics.'} />
      ) : null}

      {summary.data ? (
        <div className="grid grid--4">
          <Metric value={summary.data.total.toLocaleString()} label="Inspections" />
          <Metric value={`${summary.data.completionRate}%`} label="Completion rate"
            tone={summary.data.completionRate >= 80 ? 'ok' : 'warn'} />
          <Metric value={`${summary.data.failureRate}%`} label="Failure rate"
            tone={summary.data.failureRate > 20 ? 'danger' : summary.data.failureRate > 10 ? 'warn' : 'ok'} />
          <Metric
            ...
            <div style={{ height: 300 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={trend.data.map((p) => ({ ...p, label: bucketLabel(p.bucket) }))}>
                  ...
                  <XAxis dataKey="label" stroke={INK} fontSize={11} tickLine={false} axisLine={false} />
                  <YAxis stroke={INK} fontSize={11} tickLine={false} axisLine={false} allowDecimals={false} />
                  <Tooltip
                    contentStyle={{
                  ...
                  <Line type="monotone" dataKey="total" name="Created" stroke="var(--accent)" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="completed" name="Completed" stroke="var(--ok)" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="failed" name="Failed" stroke="var(--danger)" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          )}
              ...
              {sites.isLoading ? (
                ...
                <>
                  <div style={{ height: 240, padding: 'var(--s-4)' }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart
                        data={sites.data.slice(0, 8).map((s) => ({ name: s.name, rate: s.failureRate }))}
                        layout="vertical"
                      ...
                      >
                        ...
                        <XAxis type="number" domain={[0, 100]} unit="%" stroke={INK} fontSize={11} tickLine={false} axisLine={false} />
                        <YAxis type="category" dataKey="name" width={140} stroke={INK} fontSize={11} tickLine={false} axisLine={false} />
                        <Tooltip
                        ...
                        <Bar dataKey="rate" radius={[0, 3, 3, 0]}>
                          {sites.data.slice(0, 8).map((s) => (
                            <Cell
                              key={s.siteId}
                              fill={s.failureRate > 25 ? 'var(--danger)' : s.failureRate > 10 ? 'var(--warn)' : 'var(--ok)'}
                            />
                          ))}
                        </Bar>
                      </BarChart>
                      ...
                      <tbody>
                        {sites.data.slice(0, 10).map((site) => (
                          <tr key={site.siteId}>
                            ...
                            <td className="table__num num" style={{ color: site.failureRate > 25 ? 'var(--danger)' : undefined }}>
                              {site.failureRate}%
                            </td>
                          </tr>
                        ))}
                      ...
                      {inspectors.data.map((row) => (
                        <tr key={row.userId}>
                          ...
                          <td className="table__num num">
                            ...
                            {row.onTimeRate !== null ? `${row.onTimeRate}%` : <span className="muted">n/a</span>}
                          </td>
                        </tr>
                      ))}
                    </tbody>
